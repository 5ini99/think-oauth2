 ...
 */
define("OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME", 3600 * 24);

/**
 * The default duration in seconds of the authorization code lifetime.
 */
define("OAUTH2_DEFAULT_AUTH_CODE_LIFETIME", 60);
...
abstract class Server
{
     ...
     *setAccessToken* To avoid problems, always use lower case for persistent variable names.
     *
     * @param $name
     ...
     */
    abstract protected function setAccessToken($oauth_token, $client_id, $user_id, $expires, $scope = null);
     ...
     * @ingroup oauth2_section_4
     */
    protected function setRefreshToken($refresh_token, $client_id, $user_id, $expires, $scope = null)
    {
        return;
    }
    ...
    public function verifyAccessToken(
        ...
        {
            return $exit_not_present ? $this->errorWWWAuthenticateResponseHeader(OAUTH2_HTTP_BAD_REQUEST, $realm,
        ...
        if ($token === null) {
            return $exit_invalid ? $this->errorWWWAuthenticateResponseHeader(OAUTH2_HTTP_UNAUTHORIZED, $realm,
                OAUTH2_ERROR_INVALID_TOKEN, 'The access token provided is invalid.', null, $scope) : false;
        }

        // Check token expiration (I'm leaving this check separated, later we'll fill in better error messages)
        if (isset($token["expires"]) && time() > $token["expires"]) {
            return $exit_expired ? $this->errorWWWAuthenticateResponseHeader(OAUTH2_HTTP_UNAUTHORIZED, $realm,
                OAUTH2_ERROR_EXPIRED_TOKEN, 'The access token provided has expired.', null, $scope) : false;
        }

        // Check scope, if provided
        // If token doesn't have a scope, it's null/empty, or it's insufficient, then throw an error
        if ($scope && (!isset($token["scope"]) || !$token["scope"] || !$this->checkScope($scope, $token["scope"]))) {
            return $exit_scope ? $this->errorWWWAuthenticateResponseHeader(OAUTH2_HTTP_FORBIDDEN, $realm,
                OAUTH2_ERROR_INSUFFICIENT_SCOPE,
                'The request requires higher privileges than provided by the access token.', null, $scope) : false;
        }
    ...
    public function grantAccessToken($input = [])
    {
        ...
        $input = empty($input) ? filter_input_array(INPUT_POST, $filters) : filter_var_array($input, $filters);
        ...
        switch ($input["grant_type"]) {
            case OAUTH2_GRANT_TYPE_AUTH_CODE:
                ...
                if ($stored === null || (strcasecmp(substr($input["redirect_uri"], 0, strlen($stored["redirect_uri"])),
                            $stored["redirect_uri"]) !== 0) || $client[0] != $stored["client_id"]
                ) {
        ...
        // Check scope, if provided
        if ($input["scope"] && (!is_array($stored) || !isset($stored["scope"]) || !$this->checkScope($input["scope"],
                    $stored["scope"]))
        ) {
        ...
        $token = $this->createAccessToken($client[0], $input["scope"], $stored);
    ...
    public function getAuthorizeParams()
    {
        ...
        if (!$input["client_id"]) {
            if ($input["redirect_uri"]) {
                $this->errorDoRedirectUriCallback($input["redirect_uri"], OAUTH2_ERROR_INVALID_CLIENT, null, null,
                    $input["state"]);
            }
        ...
        if ($redirect_uri && $input["redirect_uri"]) {
            ...
            if (strcasecmp(substr($input["redirect_uri"], 0, strlen($redirect_uri)), $redirect_uri) !== 0) {
                $this->errorDoRedirectUriCallback($input["redirect_uri"], OAUTH2_ERROR_REDIRECT_URI_MISMATCH, null,
                    null, $input["state"]);
            }
        ...
        if (!$input["response_type"]) {
            $this->errorDoRedirectUriCallback($input["redirect_uri"], OAUTH2_ERROR_INVALID_REQUEST,
                'Invalid response type.', null, $input["state"]);
        }

        // Check requested auth response type against the list of supported types
        if (array_search($input["response_type"], $this->getSupportedAuthResponseTypes()) === false) {
            $this->errorDoRedirectUriCallback($input["redirect_uri"], OAUTH2_ERROR_UNSUPPORTED_RESPONSE_TYPE, null,
                null, $input["state"]);
        }
        ...
        if ($this->checkRestrictedAuthResponseType($input["client_id"], $input["response_type"]) === false) {
            $this->errorDoRedirectUriCallback($input["redirect_uri"], OAUTH2_ERROR_UNAUTHORIZED_CLIENT, null, null,
                $input["state"]);
        }

        // Validate that the requested scope is supported
        if ($input["scope"] && !$this->checkScope($input["scope"], $this->getSupportedScopes())) {
            $this->errorDoRedirectUriCallback($input["redirect_uri"], OAUTH2_ERROR_INVALID_SCOPE, null, null,
                $input["state"]);
        }
    ...
    public function finishClientAuthorization($is_authorized, $params = [])
    {
        ...
        return $this->doRedirectUriCallback($redirect_uri, $result);
    }
     ...
     * @return array
     */
    protected function createAccessToken($client_id, $scope = null, $stored = [])
    {
        $token = [
        ...
        $user_id = isset($stored['user_id']) ? $stored['user_id'] : is_login();

        // 设置accessToken
        $this->setAccessToken($token["access_token"], $client_id, $user_id,
            time() + $this->getVariable('access_token_lifetime', OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME), $scope);

        // Issue a refresh token also, if we support them
        if (in_array(OAUTH2_GRANT_TYPE_REFRESH_TOKEN, $this->getSupportedGrantTypes())) {
            $token["refresh_token"] = $this->genAccessToken();
            $this->setRefreshToken($token["refresh_token"], $client_id, $user_id,
                time() + $this->getVariable('refresh_token_lifetime', OAUTH2_DEFAULT_REFRESH_TOKEN_LIFETIME), $scope);
    ...
    private function createAuthCode($client_id, $redirect_uri, $scope = null)
    {
        ...
        $this->setAuthCode($code, $client_id, $redirect_uri,
            time() + $this->getVariable('auth_code_lifetime', OAUTH2_DEFAULT_AUTH_CODE_LIFETIME), $scope);
        return $code;
    }
     ...
     * @ingroup oauth2_error
     */
    private function errorDoRedirectUriCallback(
        $redirect_uri,
        $error,
     ...
     * @ingroup oauth2_error
     */
    private function errorWWWAuthenticateResponseHeader(
        $http_status_code,
        $realm,
        $error,
        $error_description = null,
        ...
        $result = "WWW-Authenticate: OAuth2.0 realm='" . $realm . "'";
